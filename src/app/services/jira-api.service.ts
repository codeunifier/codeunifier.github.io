...
import { JiraApiResponse, JiraTicket } from "../models/jira-ticket.model";
...
export class JiraApiService {
  async getTickets(formData: FormData): Promise<JiraApiResponse> {
    ...
    const blockingTickets = await this.fetchBlockingTickets(auth, formData.projectName, tickets.issues);
  ...
  private fetchBlockingTickets(auth: string, projectName: string, tickets: Array<JiraTicket>): Promise<JiraApiResponse> {
    ...
    const jql = `key IN (${Array.from(blockingKeys).join(',')}) AND resolved IS EMPTY ORDER BY key`;
